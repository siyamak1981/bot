...
if ($_GET['Status'] == 'OK'){
	...
	if ($result->Status == 100){
		...
		mysqli_query($conn,"UPDATE users SET account='دو ماهه', time='$time' WHERE user_id='$user'");
...
❌بعد چند دقیقه لینک باطل خواهد شد",json_encode(['inline_keyboard' => [
		...
		foreach($admins as $admin){
			SendMessage($admin,"کاربر <a href='tg://user?id=$user'>$user</a> یک اشتراک یک ماهه خریداری کرد.");
		}
	}else
